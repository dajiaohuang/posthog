...
from products.subscriptions.backend import tasks as subscription_tasks
...
def test_reconciliation_task_rotates_a_bounded_cross_team_batch(team, monkeypatch) -> None:
    ...
    for index in range(subscription_tasks._ADOPTION_RECONCILIATION_BATCH_SIZE + 1):
        ...
        ProactivePreparedArtifact.objects.for_team(owner.id).filter(id=artifact.id).update(
            ...
            - timedelta(seconds=subscription_tasks._ADOPTION_RECONCILIATION_BATCH_SIZE - index)
        )
...
def test_reconciliation_task_continues_after_one_artifact_failure(team, monkeypatch) -> None:
    ...
    monkeypatch.setattr(subscription_tasks, "reconcile_prepared_artifact", reconcile)
